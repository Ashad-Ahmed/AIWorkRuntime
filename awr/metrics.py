...
class CostTracker:
    ...
    def summary(self) -> dict[str, object]:
        ...
        for record in self._records:
            by_worker[record.worker_id] += 1
        return {
            "calls": len(self._records),
            "total_tokens": total_tokens,
            "total_cost": total_cost,
            "average_latency_ms": sum(durations) / len(durations) if durations else 0.0,
            "per_worker_calls": dict(by_worker),
        }


class RuntimeMetrics:
    @staticmethod
    def from_items(items: list[WorkItem], worker_utilization: dict[str, object] | None = None) -> dict[str, object]:
