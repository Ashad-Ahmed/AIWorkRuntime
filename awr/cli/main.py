...
def build_parser() -> argparse.ArgumentParser:
    ...
    for command in ("list", "run", "status", "metrics", "executors", "replay", "recover", "timeline"):
...
def _dispatch(args: argparse.Namespace, registry: WorkRegistry, store: SQLiteRuntimeStore) -> int:
    ...
    if args.command == "status":
        print(status_dashboard(engine.metrics(), len(engine.executors_status()), "SQLite connected", "Priority Scheduler"))
        return 0
    if args.command == "metrics":
        print(json.dumps(engine.metrics(), indent=2))
        return 0
    if args.command == "executors":
        print(json.dumps([asdict(executor) for executor in engine.executors_status()], default=list, indent=2))
        return 0
    ...
    if args.command == "list":
        for item in registry.list():
            print(panel(item.title, [("ID", item.id), ("Status", item.status.value.upper()), ("Priority", item.priority), ("Executor", item.owner_agent or "unassigned")]))
        return 0
    ...
    if args.command == "run":
        ...
        print("No runnable work." if completed is None else panel("✓ Work Executed", [("ID", completed.id), ("Title", completed.title), ("Status", completed.status.value.upper()), ("Executor", completed.owner_agent or "unassigned")]))
        return 0
...
def _start(engine: RuntimeEngine, interval: float, max_iterations: int | None) -> int:
    print(panel("AI Work Runtime v1.0", [("Storage", "SQLite"), ("Scheduler", "Priority Scheduler"), ("Executors", len(engine.executors_status())), ("Status", "Running"), ("Mode", "Watching for new work...")]))
    iterations = 0
