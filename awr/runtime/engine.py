...
from awr.executors import BaseExecutionAdapter, ExecutionAssignmentEngine, ExecutionResult, ExecutorManager


class RuntimeEngine:
    """Thin public API that composes storage, scheduling, assignment, and execution adapters."""
    ...
    def __init__(
        ...
        *,
        ...
        executors: ExecutorManager | None = None,
        workers: ExecutorManager | None = None,
        ...
        self.executors = executors or workers or ExecutorManager()
        if not self.executors.list_executors():
            self.executors.register(BaseExecutionAdapter(executor_id=self.config.executor.default_executor, max_concurrency=self.config.executor.max_concurrency))
        self.assignment = ExecutionAssignmentEngine(self.executors)
    ...
    def run_once(self) -> WorkItem | None:
        ...
        if item is None:
            return None
        executor = self.assignment.assign(item)
        item.owner_agent = executor.executor_id
        self.store.save_work_item(item)
        self.registry.event_bus.emit("ExecutorAssigned", item.id, {"executor_id": executor.executor_id})
        ...
        try:
            ...
            if result.output:
                artifact = self.artifacts.put_text(item.id, result.output, producer=executor.executor_id)
            ...
            self.assignment.release(executor.executor_id)
    ...
    def metrics(self) -> dict[str, object]:
        metrics = RuntimeMetrics.from_items(self.registry.list(), self.executors.utilization())
        metrics["costs"] = self.costs.summary()
        return metrics

    def executors_status(self) -> list[object]:
        return self.executors.list_executors()

    def workers_status(self) -> list[object]:
        return self.executors_status()
