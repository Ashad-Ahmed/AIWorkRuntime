...
from awr.executors import BaseExecutionAdapter, ExecutionResult, ExecutorManager


class TextExecutionAdapter(BaseExecutionAdapter):
    def __init__(self):
        super().__init__(executor_id="text-adapter", capabilities={"text"})
...
def test_runtime_engine_assigns_executor_records_artifact_metrics_and_replay(tmp_path):
    executors = ExecutorManager()
    executors.register(TextExecutionAdapter())
    ...
    assert engine.registry.get(item.id).artifacts[0]["producer"] == "text-adapter"
    assert engine.metrics()["costs"]["total_tokens"] == 5
    assert engine.replay()[item.id].status == "completed"
    engine.shutdown()


def test_assignment_rejects_incompatible_executors(tmp_path):
    ...
    with pytest.raises(ValueError, match="no compatible executor"):
        engine.run_once()
